...
| Frontend shell & UX | `app/layout.tsx`, `app/page.tsx`, nav, filters, `globals.css` | 2026-09-01 | PARTIAL. Fixed: story pages server-rendered (323 -> 2,175 chars, h1 present), site-wide skip link, `<main>` on the story route. Open: `<main>` still missing on series/ship/fandom routes; 99 raw fetches with only 10 guarded, `fetchOrFail` still unused (core search has its own timeout, so this is the non-search pages). `app/page.tsx` is 2,853 lines and was not read in full. |
