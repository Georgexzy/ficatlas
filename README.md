...
A unified search engine for fanfiction. **20.7M works** indexed across AO3 (14.1M),
...
### Search & discovery
- **Unified search** in one query across AO3 (14.1M works), FanFiction.net (6.6M) and
...
- **A sentence is a search.** Type what you actually want — *"a long drarry fic where draco isn't a total douche, no smut"* — and if the literal words match nothing, the request is read against the index's own vocabulary and run as `ship:"Draco Malfoy/Harry Potter" fandom:"Harry Potter - J. K. Rowling" -tag:"Smut"`. That exact sentence returned **0 results** before this and hundreds after. It is offered, never silently applied: the page says what it searched for instead, with your own words one click away, and the address bar ends up holding the query that actually ran so the link is shareable
...
- **Browse by pairing** — 6,165 ship pages (`/ships`, `/ship/<slug>`), the other route in, and the one where a cross-archive index has something to say that no single archive can. AO3's tag pages cover AO3; `/ship/draco-malfoy-harry-potter` puts 50 AO3, 50 FanFiction.net and 50 FictionAlley works for the same pairing on one page, out of 53,283 indexed. Both orders of a pairing collapse onto one page — "Draco Malfoy/Harry Potter" (47,460 works), "Harry Potter/Draco Malfoy" (1,541) and "Harry Potter/ Draco Malfoy" (46) are one ship, not three thin duplicates. The slug is alphabetical so the URL is stable, while the heading and its search link use the spelling the archives actually use. Romantic pairings only: AO3's `/` and `&` mean different things to the people reading, and they slugify identically, so building both would merge a ship with a friendship
...
- **Hub pages are the front door, so they carry the search.** Most of what search engines send lands on a `/ship/` or `/fandom/` page rather than the home page, and only a small fraction of those visitors ever ran a search — because the page offered a *link* to the search box. It now carries the box itself: a plain GET form, so it needs no JavaScript and a crawler sees a real form, scoped to that pairing or fandom. Under it are that hub's **own** most common qualities rather than a fixed list — `Drarry` offers Angst · Fluff · Romance · Humor · Slow Burn, `Remus/Sirius` offers Marauders Era, `Castiel/Dean` offers AU – Victorian — sampled from its works at build time and filtered against the hub's own names and nicknames, AO3's `- Freeform` duplicates, the writing process, and any word people write more often than they tag. Length and completion stay in their own row, because those are true of every hub
- **The archive that actually holds the works leads the page.** Sections were ordered by how many works each archive contributed to a *capped* listing, so any two that filled their quota tied and fell back to dictionary order — the Drarry hub opened with FictionAlley, which holds **88** of its 53,283 works against AO3's 51,894. AO3 looked smallest precisely because it is the archive the content filter trims most
...
### Running it (the `/admin` side)

Four areas, named for the work rather than for what was built: **Index**,
**Audience**, **Outreach**, **Moderation**.

- **Needs attention** — the Index tab leads with what is actually wrong, worst first, ranking stale jobs and metadata gaps in one list. They are comparable only once both are expressed as what they cost: a coverage gap is ranked by how many works it denies a reader, weighted by what the field is *for* (a work with no characters cannot be found by the filter people use most; one with no kudos merely sorts late), and a job by how far past its own staleness limit it is. Every figure in it was already on that page before, ordered by nothing — which is how the weekly cross-archive popularity pass sat broken for **thirteen days** with the evidence on screen the whole time
- **Job evidence, not heartbeats** — each background job is judged by something it *left behind* (a build timestamp, a watermark), because a heartbeat says "I ran" and evidence says "I achieved something". Alongside them, queue depths, which answer the opposite question: not "did it run" but "is it keeping up"
- **Outreach is one screen** — the site's distribution problem is that nobody has heard of it, and the audience already asking the question it answers is the fic-finder threads (r/FanFiction's Lost Fic flair, r/AO3's Fic/Work Search). A worklist fetches those posts, runs each through the same extractor the search box uses, and ranks by how *few* works the answer holds — four can be read, five thousand cannot. Picking one hands it straight to the finder, already extracted, with the post it answers named above the results. It posts nothing and holds no credential that could: the reply is text on a clipboard and a person decides. Posts whose reply would carry a link the subreddit's own rules forbid are stored with the reason, sorted last, and labelled rather than hidden
- **Traffic counts journeys, not just events** — where a visit *starts* and what it does next, because a hub page appearing as two views says nothing about whether the visitor it brought went on to use the site. Plus a **scraper flag**: everything in the traffic table was *served*, so a named scraper appearing in it is the record of an edge rule that did not fire, and the panel says so with the days, the peak arrival rate and the paths needed to write a new one
- **Retention without identification** — the visitor hash is salted with the date, so nothing links one day to the next and returning readers were structurally invisible. The browser answers instead and hands over only the answer: it keeps its own last-seen date and sends one of three words. What arrives is a bucket, not an identifier — the same three values for everybody, joinable to nothing, and useless for telling two people apart

### Not being scraped to death

The index is the thing of value here and it is served from a home connection,
so the defences are at the edge, where a refused request costs no bandwidth.
They are aimed at clients, never at readers, and every one is reversible.

- **Agent browsers, as a class** — a scrape once took **98.3% of a day's origin requests** (87,330 of 88,860) from one agent browser across 5,863 IPv6 addresses, executing JavaScript and therefore counting as 23,993 readers in the traffic panel. Blocking it by name worked until eleven days later, when the same signature arrived under a different one. The rule names the class — Headless, the automation drivers, the scraping-as-a-service products — and exempts Cloudflare-verified crawlers, so no amount of string matching in it can cost us Googlebot
- **A challenge that does not catch readers** — a residential-proxy botnet walking `/story/` (19,705 addresses, one request each) is met with a managed challenge. Measured afterwards, that challenge was also costing real visitors at the last step of the funnel — sessions that opened a story page fell 75 → 29 — so it now exempts anyone arriving with a referrer from this site, which is the one signal the botnet does not have. Verified: with a referrer 200, without one 403
- **`robots.txt` is policy, the edge is enforcement** — the crawlers that read the file and then do their own thing are refused the 20.5M-page `/story/` tail while keeping the home page, both index pages and all 11,190 hub pages, which are the pages the site actually wants indexed
